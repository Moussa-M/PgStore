        ...
        <nav class="navbar navbar-default">
            <div class="nav-container">
                <div class="navbar-header" style="display: inline-flex;">
                    <a class="navbar-brand">{{$t("new_invoice")}}</a>
                </div>
                                            ...
                                            <draggable @change="updateStocksOrder()" v-model="invoice.stocks" tag="tbody" v-if="invoice.stocks.length > 0">
                                                ...
                                                <tr style="cursor: pointer;" class="new-row" v-for="(stock,index) in invoice.stocks" :ref="stock.id" :key="stock.id">
                                                    ...
                                                    <td :class="$i18n.locale=='ar'?'text-right':'text-left'">
                                                        <div v-if="!stock.edit">{{stock.total_qte}} {{$t(stock.product.unit.toLowerCase())+" "}}<span style="direction:ltr" v-html="getStockQteDesc(stock)"></span></div>
                                                        <div v-else>
                                                            <div class="row">
                                                                <div class="col-md-6" style="padding:5px">
                                                                    <div class="form-group">
                                                                        <div class="input-group">
                                                                            <input v-tooltip="{content: 'The number of boxes',delay: {show: 750,hide: 10,},}" type="number" :class="errors['pack']?'error_border':''" min="1" step="1" oninput="validity.valid||(value='1');" @change="stock.pack = parseFloat(stock.pack.toString())" @keyup.enter="addStockToInvoice()" class="form-control border-input animated" v-model="stock.pack" :placeholder="$t('pack_count')" value="1">
                                                                ...
                                                                <div class="col-md-6" style="padding:5px" v-if="stock.product.is_bulk_sell">
                                                                    <div class="form-group">
                                                                        <div class="input-group">
                                                                            <input v-tooltip="{content: 'The number of items in one box',delay: {show: 750,hide: 10,},}" :class="errors['extra_units']?'error_border':''" @keyup.enter="addStockToInvoice()" v-model="stock.extra_units" type="number" dir="ltr" :max="stock.unit=='piece'?parseInt(stock.packing)-1:parseFloat(stock.packing)-0.01" :min="stock.unit=='piece'?1:0.01" :step="stock.unit=='piece'?1:0.01" :ref="'extra_units'+stock.id" @keyup="extraUnitChangedList(stock)" class="form-control border-input animated" :placeholder="stock.unit=='piece' ? $t('extra_units'):$t('extra_weight')">

                                                                            <span style="padding: 3px;font-size: 12px;" class="input-group-addon">{{$t(stock.unit.toLowerCase())}}</span>
                                                                        </div>
                                                    ...
                                                    <td :class="$i18n.locale=='ar'?'text-left':'text-right'">
                                                        <button v-if="!stock.edit" @click="deleteStock(stock.id,$event)" class="btn btn-danger btn-icon">
                                                            <i class="ti-trash"></i>
                                                        </button>
                                                ...
                                                <td :class="$i18n.locale=='ar'?'text-right':'text-left'">
                                                    <div class="row">
                                                        <div class="col-md-6" style="padding:5px">
                                                            <div class="form-group">
                                                                <div class="input-group">
                                                                    <input v-tooltip="{content: 'The number of boxes',delay: {show: 750,hide: 10,},}" type="number" :class="new_errors['pack']?'error_border':''" min="1" step="1" oninput="validity.valid||(value='1');" @change="new_stock.pack = parseFloat(new_stock.pack.toString())" @keyup.enter="addStockToInvoice()" class="form-control border-input animated" v-model="new_stock.pack" :placeholder="$t('pack_count')" value="1">
                                                        ...
                                                        <div class="col-md-6" style="padding:5px">
                                                            <div class="form-group">
                                                                <div class="input-group">
                                                                    <input v-tooltip="{content: 'The number of items in one box',delay: {show: 750,hide: 10,},}" :class="new_errors['packing']?'error_border':''" @keyup.enter="addStockToInvoice()" v-model="new_stock.extra_units" type="number" dir="ltr" :max="new_stock.unit=='piece'?parseInt(new_stock.packing)-1:parseFloat(new_stock.packing)-0.01" :min="new_stock.unit=='piece'?1:0.01" :step="new_stock.unit=='piece'?1:0.01" ref="extra_unit" @keyup="extraUnitChanged()" class="form-control border-input animated" :placeholder="new_stock.unit=='piece' ? $t('extra_units'):$t('extra_weight')">

                                                                    <span style="padding: 3px;font-size: 12px;" class="input-group-addon">{{$t(new_stock.unit.toLowerCase())}}</span>
                                                                </div>
                          ...
                          >
                            ...
                            <div class="row" :class="$i18n.locale=='ar'?'text-left':'text-right'" style="margin-bottom:50px">
                                ...
                                <div style="margin-top:20px" :class="$i18n.locale=='ar'?'text-left':'text-right'">
                                    <button v-if="invoice.stocks.length>0" @click="saveInvoie()" type="submit" class="btn btn-lg">{{$t("save_invoice")}}</button>
                                </div>
                                <div class="clearfix"></div>
                            </div>
                        </div>
...
export default {
    ...
    created: function () {
        ...
        this.invoice = new Invoice([], this.customers[0].id);
        this.invoice.stocks = [];
        ...
        this.new_stock = {
            id: uuid(),
            product_id: null,
            supplier_id: this.customers[0].id,
    ...
    methods: {
        getStockQteDesc(stock) {
            ...
            return stock.product.is_bulk_sell ? _.join(["(", stock.pack, "x", stock.packing, stock.extra_units ? '+' + stock.extra_units : "", ") "], '') : ""
        },
        extraUnitChanged() {
        ...
        onHit: function (stock, data) {
            ...
            if (stocks.length > 0) {
                ...
                this.new_stock.product_id = item.id;
  ...
  <div class="media-body">
    <h5 class="media-heading">${item.name}</h5>
        ...
        addStockToInvoice() {
            ...
            if (!this.new_stock.extra_units && this.new_stock.product.is_bulk_sell && this.new_stock.extra_units != 0) {
                this.errors["extra_units"] = this.$t("extra_required");
            }
            ...
            if (Object.entries(this.new_errors).length == 0) {
                ...
                this.new_stock.total_qte = (parseFloat(this.new_stock.pack) * parseFloat(this.new_stock.packing)) + parseFloat(this.new_stock.extra_units)
                this.new_stock.available_qte = (parseFloat(this.new_stock.pack) * parseFloat(this.new_stock.packing)) + parseFloat(this.new_stock.extra_units)
        ...
        checkUpdateStock(stock) {
            return new Promise((resolve, reject) => {
                ...
                if (!stock.extra_units && stock.product.is_bulk_sell && stock.extra_units != 0) {
                    this.errors["extra_units"] = this.$t("extra_required");
                }
                if (!stock.sell_price) {
                ...
                if (Object.entries(this.errors).length == 0) {
                    stock.disabled = true;
                    stock.edit = false;
                    stock.total_qte = (parseFloat(stock.pack) * parseFloat(stock.packing)) + parseFloat(stock.extra_units);
                    stock.available_qte = (parseFloat(stock.pack) * parseFloat(stock.packing)) + parseFloat(stock.extra_units);
        ...
        updateStock(stock) {
            ...
            if (Object.entries(this.errors).length == 0) {
                ...
                stock.total_qte = (parseFloat(stock.pack) * parseFloat(stock.packing)) + parseFloat(stock.extra_units);
                stock.available_qte = (parseFloat(stock.pack) * parseFloat(stock.packing)) + parseFloat(stock.extra_units);
                stock.exp_date = stock.exp_date ? moment(stock.exp_date).format() : "";
                console.log("stock");
                console.log(stock);
                let index = _.findIndex(this.invoice.stocks, {
                    id: stock.id
            ...
            this.invoice.stocks = _.map(this.invoice.stocks, s => {
                if (s.id == stock.id) {
                    ...
                    if (stocks.length > 0) {
                        ...
                        s.total_qte = s.pack * s.packing + s.extra_units;
                        s.available_qte = s.pack * s.packing + s.extra_units;

                        s["packing"] = item.packing;
                        s["unit"] = item.unit;
                        s["edit"] = true;
                        s["disabled"] = true;
                    } else {
            ...
            swal({
                ...
                if (willDelete) {
                    ...
                    setTimeout(() => {
                        ...
                        _.remove(this.invoice.stocks, function (o) {
                            return o.id == id;
                        });
