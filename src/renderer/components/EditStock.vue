        ...
        <nav class="navbar navbar-default">
            <div class="nav-container">
                <div class="navbar-header" style="display: inline-flex;">
                    <a class="navbar-brand">{{$t("edit_stock_of",{product:product.name})}}</a>
                </div>
                        ...
                        <div class="content" style="padding: 0px">
                                        ...
                                        <div class="row">
                                            ...
                                            <div v-if="product.is_bulk_sell" class="col-md-3">
                                                <div class="form-group">
                                                    ...
                                                    <div class="input-group">
                                                        <input type="number" dir="ltr" class="form-control border-input animated no-cursor" :placeholder="stock.unit=='piece' ? $t('extra_units'):$t('extra_weight')" disabled :value="(parseFloat(stock.pack*product.packing)+parseFloat(stock.extra_units))-(parseFloat(stock.total_qte)-parseFloat(stock.available_qte))">
                            ...
                            <div class="row " style="margin-bottom:50px;margin-left: 0px;margin-right: 0px;">
                                <div :class="$i18n.locale=='ar'?'text-left':'text-right'">
                                    <button @click="save" type="submit" class="btn btn-lg">{{$t("save")}}</button>
                                </div>
...
export default {
    ...
    created() {
        ...
        this.stock = JSON.parse(JSON.stringify(store.getters.getStock(this.id)));
        ...
        this.invoice = store.getters.getInvoice(this.stock.invoice_id);
        this.is_supplier_selected = true;
        this.product = JSON.parse(
            JSON.stringify(store.getters.getProduct(this.stock.product_id))
        );
    ...
    methods: {
        ...
        save: function (e) {
            ...
            // if (!this.stock.packing) {
            //     this.errors.push(this.$t("packing_required"));
            // }
            ...
            if (this.errors.length == 0) {
                let new_total = this.stock.pack * this.stock.packing + parseFloat(this.stock.extra_units);
                let sold_amount = this.stock.total_qte - this.stock.available_qte;
                if (sold_amount <= new_total) {
                    let stock_clone = Object.assign({}, this.stock);
                    stock_clone.available_qte = new_total - sold_amount;
                    stock_clone.total_qte = new_total;
                    ...
                    stock_clone.available_qte = (stock_clone.pack * stock_clone.packing + stock_clone.extra_units)-sold_amount;
                    stock_clone.exp_date = stock_clone.exp_date ?
                        moment(stock_clone.exp_date).format() :
                        "";
                    console.log("stock_clone.sell_price , stock_clone.buy_price")
                    console.log(stock_clone.sell_price, stock_clone.buy_price)
        ...
        updateStock() {
            let sold_amount = this.stock.total_qte - this.stock.available_qte;
            if (this.buy_price_type == 'all') {
                this.stock.buy_price = parseFloat(this.stock.buy_price / ((this.stock.pack * this.product.packing) + this.stock.extra_units))
            } else if (this.buy_price_type == 'pack') {
                this.stock.buy_price = parseFloat(this.stock.buy_price / this.product.packing)
            }
            if (this.sell_price_type == 'all') {
                this.stock.sell_price = parseFloat(this.stock.sell_price / ((this.stock.pack * this.product.packing) + this.stock.extra_units))
            } else if (this.sell_price_type == 'pack') {
                this.stock.sell_price = parseFloat(this.stock.sell_price / this.product.packing)
            }
            ...
            this.stock.available_qte = (parseFloat(this.stock.pack) * parseFloat(this.stock.packing) + parseFloat(this.stock.extra_units))-sold_amount;
            this.stock.exp_date = this.stock.exp_date ?
                moment(this.stock.exp_date).format() :
                "";
            store.dispatch("updateStock", this.stock);
            console.log("this.stock");
            console.log(this.stock);
            this.goBack();
