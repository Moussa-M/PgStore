<template>
  ...
  <div>
    <div
      :class="removeable ? 'removeable':''"
      @click.prevent="selectImage"
      class="avatar"
      :style="style"
    ><div style="position:relative;z-index:999998"><div v-if="img!=null" @click="removeImage($event)" id="close"></div></div>
      <div id="container">
...
export default {
  ...
  computed: {
    ...
    style () {
      var fontSize = this.width / 3
      return {
        'width': this.width + 'px',
        'height': this.height + 'px',
        'border-radius': this.radius + '%',
        'font-size': fontSize + 'px',
        'cursor': this.changeable ? 'pointer' : 'default',
        'background-color':
          this.color === '#3F3F3F' ? this.toColor(this.fullname) : this.color
      }
    },
    hasImage () {
  ...
  methods: {
    ...
    loadImage (impath, emiturl) {
      ...
      imageObject.onerror = function (err) { console.log(err); throw err }
    ...
    toColor (str) {
      ...
      if (color.length < 6) {
        color = color + Array.apply(null, {length: 6 - color.length}).fill(0).join('')
      }
